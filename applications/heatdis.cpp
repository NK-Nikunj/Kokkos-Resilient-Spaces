...
namespace heatdis {
    ...
    {
        typedef Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>
            range_policy;
        typedef Kokkos::MDRangePolicy<Kokkos::DefaultHostExecutionSpace,
            Kokkos::Rank<2>>
            mdrange_policy;

        /* set all of the data to 0 */
        Kokkos::parallel_for(
            "init_h", mdrange_policy({0, 0}, {nbLines, M}),
            KOKKOS_LAMBDA(const int i, const int j) { h((i * M) + j) = 0; });
        ...
        if (rank == 0)
        {
            int j = ceil(M * 0.9);
            Kokkos::parallel_for(
                "init_rank0", range_policy((int) (M * 0.1), j),
                KOKKOS_LAMBDA(const int i) { h(i) = 100; });
        }
    }
        ...
        Kokkos::View<double*> g, Kokkos::View<double*> h)
    {
        ...
        Kokkos::parallel_for(
            "copy_g", mdrange_policy({0, 0}, {nbLines, M}),
            KOKKOS_LAMBDA(
                const int i, const int j) { h((i * M) + j) = g((i * M) + j); });
